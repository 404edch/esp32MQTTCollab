...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if "alerta de temperatura" in dados:
            print(f" -> Alerta de temperatura: {dados['alerta de temperatura']}")
        if "temperatura" in dados:
            print(f" -> Temperatura: {dados['temperatura']} C")
        if "alerta de umidade" in dados:
            print(f" -> Alerta de umidade: {dados['alerta de umidade']}")
        if "umidade" in dados:
            print(f" -> Umidade: {dados['umidade']}")
        if "alerta de movimento de carga" in dados:
            print(f" -> Alerta de movimento de carga: {dados['alerta de movimento de carga']}")
        if "proximidade" in dados:
            print(f" -> Proximidade: {dados['proximidade']} cm")
...
def main():
    ...
    print(" lock -> Pausa a atualização de estados")
    print(" unlock -> Resume a atualização de estados")
    print(" emergencia_comecar -> Ativa o modo de emergencia manualmente")
    print(" emergencia_parar -> Desativa o modo de emergencia manualmente")
